...
- النتيجة: `DONE`.
- Commit التنفيذ: `d877b4d`.
- فحص الصياغة لكل وحدات Core: ناجح.
- فحص عقود الوحدات (`scripts/typecheck.mjs`): ناجح.
- الاختبارات: `44/44` ناجحة.
- تشمل الأدلة: SHA-256، عدم تعديل bytes الأصلية، كشف التكرار المتزامن، نسخ الملفات،
  العلاقات بالـStable IDs، Migration من v0/v1/v2 إلى v3، Reopen، Recovery بعد فشل
  الحفظ، ورفض محتوى تالف حتى لو بقي حجمه مطابقاً.
- تحقق IndexedDB الفعلي داخل المتصفح يبقى ضمن Browser smoke/التكامل اللاحق؛ عقد
  التخزين مختبر الآن ببديل Memory مستقل، ولا يوجد ادعاء غير مختبر حول MatePad.
