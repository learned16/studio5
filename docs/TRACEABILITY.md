...
| `S5-QA-AUTOPILOT-003` | تحقق مسار التسليم باشتراك Codex الحالي فقط | OPS-AUTOPILOT-003 | IN PROGRESS — LOCAL VALIDATION | Task card + reusable subscription-only contract + focused regression tests؛ الإكمال ينتظر B mutation guard وDraft PR وGitHub CI؛ لا API key ولا Full access ولا auto-merge |
