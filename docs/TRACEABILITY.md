...
| `S5-QA-UTF8-001` | Changed-file UTF-8/mojibake prevention | OPS-QUALITY-UTF8-001 | IN PROGRESS | Recurring PR #29/#30/#33 repair evidence; deterministic delivery guard is bounded to newly introduced changed text; Automation Optimization Freeze remains active and routing returns to Phase 4.5 after merge |
...
| `S5-UX-P45-STUDY-NOTES-001` | Read-only Study subject notes projection | P4.5-UX-IMPLEMENTATION-011 | LOCAL IMPLEMENTATION VERIFIED — PARENT REVIEW PENDING | frozen `listNotes({subjectId})` facade; inline ordered read-only projection with loading, empty, error, Retry, and stale-response protection |
| `S5-UX-P45-STUDY-FILES-001` | Read-only Study subject files projection | P4.5-UX-IMPLEMENTATION-012 / PR #31 | COMPLETE / MERGED | frozen surface-local `searchLibrary({query:"",subjectId,targetKinds:["file-artifact"],limit:500})` facade; inline ordered read-only projection with loading, empty, error, Retry, and stale-response protection |
| `S5-UX-P45-STUDY-FILE-METADATA-001` | Read-only Study file metadata detail | P4.5-UX-IMPLEMENTATION-013 | LOCAL IMPLEMENTATION VERIFIED — PARENT REVIEW PENDING | frozen surface-local `getFileArtifact(artifactId)` facade; projected metadata with loading, ready, missing, recoverable error, Retry, Close, and stale-response protection |
| `S5-UX-P45-STUDY-FILE-VERSIONS-001` | Read-only Study file versions metadata | P4.5-UX-IMPLEMENTATION-014 | LOCAL IMPLEMENTATION VERIFIED — PARENT REVIEW PENDING | frozen surface-local `listFileVersions({artifactId})` facade; nested ordered version projection with loading, ready, empty, recoverable error, Retry, and stale-response protection |
