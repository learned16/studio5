...
| `S5-UX-P45-STUDY-FILES-001` | Read-only Study subject files projection | P4.5-UX-IMPLEMENTATION-012 | LOCAL IMPLEMENTATION VERIFIED — PARENT REVIEW PENDING | frozen surface-local `searchLibrary({query:"",subjectId,targetKinds:["file-artifact"],limit:500})` facade; inline ordered read-only projection with loading, empty, error, Retry, and stale-response protection |
