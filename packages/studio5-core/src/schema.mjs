...
const COLLECTIONS_V5 = Object.freeze([
  ...COLLECTIONS_V4,
  "lectureCaptures",
  "lectureCloseouts",
  "captureResolutions",
...
const COLLECTIONS_V6 = Object.freeze([
  ...COLLECTIONS_V5,
  "resourceMarkers",
]);

const COLLECTIONS_V7 = Object.freeze([
  ...COLLECTIONS_V6,
  "offlineOperations",
]);
...
function migrateVersion4(input, now) {
  const entities = emptyEntities(COLLECTIONS_V5);
...
function migrateVersion5(input, now) {
  const entities = emptyEntities(COLLECTIONS_V6);
  for (const collection of COLLECTIONS_V5) {
    entities[collection] = Array.isArray(input?.entities?.[collection])
      ? structuredClone(input.entities[collection])
      : [];
  }
...
function migrateVersion6(input, now) {
  const entities = emptyEntities(COLLECTIONS_V7);
  for (const collection of COLLECTIONS_V6) {
    entities[collection] = Array.isArray(input?.entities?.[collection])
      ? structuredClone(input.entities[collection])
      : [];
  }
...
function migrateVersion7(input, now) {
  const entities = emptyEntities();
  for (const collection of COLLECTIONS_V7) {
    entities[collection] = Array.isArray(input?.entities?.[collection])
      ? structuredClone(input.entities[collection])
      : [];
  }
